...
def run(config):
  ...
  if config['sample_npz']:
    ...
    out_dir = os.path.join(config['samples_root'], experiment_name)
    os.makedirs(out_dir, exist_ok=True)
    print('Saving to {}...'.format(out_dir))
    for _ in trange(int(np.ceil(config['sample_num_npz'] / float(G_batch_size)))):
      with torch.no_grad():
        images, labels = sample()
      for img in images.cpu():
        img = np.uint8(255 * (img.permute([1, 2, 0]).numpy() + 1) / 2.)
        img = Image.fromarray(img)
        img.save('{}/{}.png'.format(out_dir, count))
        count += 1
